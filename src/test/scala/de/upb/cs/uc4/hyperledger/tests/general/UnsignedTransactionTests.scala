...
import de.upb.cs.uc4.hyperledger.utilities.helper.{ CryptoHelper, Logger, StringHelper }
...
class UnsignedTransactionTests extends TestBase {
  ...
  override def beforeAll(): Unit = {
    ...
    val id: String = "701"
    prepareUser(id)
    val mat1 = TestDataMatriculation.validMatriculationData1(id)
    TestSetup.establishExistingMatriculation(initializeMatriculation(), initializeOperation(id), id, mat1)
  }
  ...
  "The ConnectionCertificate" when {
    "querying for an unsigned proposal" should {
      "return an unsigned proposal" in {
        ...
        val jsonRegulation = TestDataExaminationRegulation.validExaminationRegulation("UnsignedTransactionTests_ComputerScience", Seq(TestDataExaminationRegulation.getModule("M1")), isOpen = true)
