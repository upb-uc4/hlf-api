  ...
  "The ScalaAPI for Certificate" when {
    "invoking getCertificate" should {
      ...
      for ((statement: String, data: String) <- testData) {
        s"$statement" in {
          ...
          TestHelper.testTransactionException("proposeTransaction", () => chaincodeConnection.getCertificate(data))
        }
      }
    }
    ...
    "invoking addCertificate" should {
      ...
      for ((statement: String, enrollmentId: String, certificate: String) <- testData) {
        s"$statement" in {
          Logger.info("Begin test: " + statement)
          TestHelper.testTransactionException("proposeTransaction", () => chaincodeConnection.addCertificate(enrollmentId, certificate))
        }
      }
    }
    ...
    "invoking updateCertificate" should {
      ...
      for ((statement: String, enrollmentId: String, certificate: String) <- testData) {
        s"$statement" in {
          Logger.info("Begin test: " + statement)
          TestHelper.testTransactionException("proposeTransaction", () => chaincodeConnection.updateCertificate(enrollmentId, certificate))
        }
      }
    }
  }
